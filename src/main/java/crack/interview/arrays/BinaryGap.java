...
/*
Initial solution done for facebook which is wrong
because trailing zeros are also counted.
...
public class BinaryGap {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(findBinaryGap(value.toBinaryString(value)));

    }

    private static Integer findBinaryGap(String string) {
        Integer result = 0;
        String[] splitVal = string.split("1");
        Integer it = splitVal.length;
        // edge case if there is a zero at the end
        if (string.charAt(string.length() - 1) == '0') {
            it = splitVal.length - 1;
        }
        for (int i = 0; i < it - 1; i++) {
            if (result < splitVal[i].length()) {
                result = splitVal[i].length();
            }
        }
        return result;
    }
